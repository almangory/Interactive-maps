...
import { FileText, Layers, Ruler, BarChart3, Hash, ArrowUpDown, Eye, MapPin, Filter, Search, CheckCircle2, AlertTriangle } from 'lucide-react';
import { KMLFeatureItem } from '../types';
import { EXACT_APPROVED_COLOR_MAP } from '../utils/colorCompliance';
...
export const PermitLengthChart: React.FC<PermitLengthChartProps> = ({
  ...
  const permitGroups = useMemo(() => {
    ...
    items.forEach(it => {
      const pNo = it.permitNo && it.permitNo.trim() ? it.permitNo.trim() : 'بدون رقم تصريح (غير محدد)';
      if (!map[pNo]) {
        map[pNo] = {
          permitNo: pNo,
          count: 0,
  ...
  const stats = useMemo(() => {
    ...
    const totalPermits = permitGroups.filter(p => !p.permitNo.includes('بدون رقم')).length;
    let sumLen = 0;
    let maxLen = 0;
    let longestPermit = '-';
    ...
    permitGroups.forEach(p => {
      ...
      } else if (p.totalLengthMeters > maxLen) {
  ...
  const chartData = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortBy === 'permit-name') return a.permitNo.localeCompare(b.permitNo);
      return 0;
    });
    ...
    return list.map((p, idx) => ({
      ...p,
      rank: idx + 1,
      displayName: p.permitNo.length > 20 ? p.permitNo.substring(0, 18) + '...' : p.permitNo,
      chartLength: unit === 'km' ? p.totalLengthKm : Math.round(p.totalLengthMeters)
    }));
  }, [permitGroups, statusFilter, searchQuery, sortBy, topLimit, unit]);
      ...
      return (
        <div className="bg-slate-900/95 border border-slate-700 text-white p-3 rounded-xl shadow-2xl backdrop-blur-md text-xs space-y-1.5 z-50 pointer-events-none min-w-[200px]">
          ...
          <div className="text-slate-300 flex justify-between">
            ...
            <strong className="text-emerald-400 font-mono">{unit === 'km' ? `${data.totalLengthKm} كم` : `${data.totalLengthMeters.toLocaleString('ar-SA')} متر`}</strong>
          </div>
          <div className="text-slate-300 flex justify-between">
    ...
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            توزيع وتحليل الأطوال المنجزة والجارية لكل تصريح حفر مسجل بالخريطة.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 px-3 py-1 rounded-xl border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
            ...
            <span>إجمالي التصاريح: {stats.totalPermits}</span>
          </span>
          {stats.unpermittedCount > 0 && (
            <span className="text-xs font-bold bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 px-3 py-1 rounded-xl border border-rose-200 dark:border-rose-800 flex items-center gap-1">
              ...
              <span>بدون تصريح: {stats.unpermittedCount} عنصر</span>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-xs font-bold bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 px-2.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer"
          >
      ...
      {chartData.length > 0 ? (
        ...
        <div className="p-8 text-center text-xs text-slate-400 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
          لا توجد بيانات تصاريح مطابقة للفلترة الحالية.
        </div>
      )}
    </div>
  );
